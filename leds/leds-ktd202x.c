 ...
 */
 ...
 /*
  * The KTD202x series of chips do not handle i2c reads correctly.
  * This driver caches internal register states instead.
  */
 ...
 {
	 return container_of(led_cdev, struct ktd202x_channel, cdev);
 }
				 ...
				 unsigned int data)
 {
	 ...
	 result = regmap_update_bits(regmap,
					 ...
					 output);
	 if (result < 0)
		 pr_err("ktd202x: regmap_update_bits failed: %d.\n", result);
				 ...
				 unsigned int *out)
 {
	 ...
	 if (result < 0) {
		 pr_err("ktd202x: regmap_read failed %d.\n", result);
		 return result;
	 }
 ...
 static int ktd202x_reset(struct ktd202x_context *ctx)
 {
	 ...
	 /* Reset: write Reg0[2:0] = 0b111, then wait 300us.
	  * During reset, chip may NAK so result is ignored.
	  */
	 ...
	 /* Force device always on */
	 ret = regmap_write(ctx->regmap, ktd202x_regs[REG0_ENABLE].addr, 0x03);
	 if (ret < 0) {
		 dev_err(&ctx->client->dev, "Could not set chip power management: %d", ret);
		 return ret;
	 }
 ...
 static enum led_brightness ktd202x_brightness_get(struct led_classdev *led_cdev)
 {
	 ...
	 /* If channel disabled, brightness is 0 */
	 if (result == 0)
		 return 0;
 ...
 static int ktd202x_init(struct ktd202x_context *ctx)
 {
	 ...
	 /* Reset device before LED framework queries brightness */
 ...
 /*
  * Parse "reg=value;" string where reg is register name and value in 0-255.
  */
 ...
 /*
  * Parse "reg=val;reg=val;...;reg=val;" command to update register values.
  */
						...
						const char *buf, size_t count)
 {
	 ...
	 while (*str && n < ARRAY_SIZE(parsed)) {
		 ...
		 if (!str)
			 return -EINVAL;
		 dev_dbg(dev, "reg=%d, value=%d", parsed[n - 1].reg, parsed[n - 1].def);
	 }
 ...
 static DEVICE_ATTR(ch1_enable, 0644, ktd202x_show_register, ktd202x_store_register);
 static DEVICE_ATTR(ch2_enable, 0644, ktd202x_show_register, ktd202x_store_register);
 static DEVICE_ATTR(ch3_enable, 0644, ktd202x_show_register, ktd202x_store_register);
 static DEVICE_ATTR(ch4_enable, 0644, ktd202x_show_register, ktd202x_store_register);
 ...
 /*
  * probe 関数のシグネチャを i2c_client のみのものに変更します。
  * id 引数は削除し、client->name を利用するなどしてください。
  */
 static int ktd202x_probe(struct i2c_client *client)
 {
	 struct device *dev = &client->dev;
	 struct ktd202x_context *ctx;
	 int err;
	 ...
	 /* devm_device_add_group() を用いて sysfs 属性グループを登録 */
	 err = devm_device_add_group(dev, &ktd202x_dev_attr_group);
	 if (err) {
 ...
 /*
  * i2c_driver 構造体は従来の .probe を使用します。
  */
